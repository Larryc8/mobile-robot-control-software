...
class ImageViewer(QMainWindow):
    save_selected_points = pyqtSignal(list)

    def __init__(self, ylm_map_filepath=None):
        super().__init__()
    ...
    def save_points(self):
        a = self.graphics_view.getPointsPath()
    ...
    def load_image(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Open Image File", "", "Image Files ( *.pgm)"
        )
        file_path = "./map2.pgm"
    ...
    def load_map(self, file_path):
        ## LAUNCH AN EXEPTION WHEN IS NOT A VAILID FILE
        try:
            ...
            self.display_image(f"{root_file_path}/{image}")
...
class Example(QGraphicsView):
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            self.pointsToCheck.append({"color": Qt.red, 'x': obj.x(), 'y': obj.y(), 'id': sync_id})
    ...
    def load_stored_points(self):
        obj = QRectF(10, 10, 10, 10, 10)
        self.squares.append(
            {"color": Qt.red, "object": obj, "id": datetime.now().timestamp()}
        )
        pass
