...
# cmapGR = LinearSegmentedColormap.from_list('custom', ['red', 'yellow','green', 'blue'])
...
# Stack data into [N,2] dimension for RBFInterpolator 
    ...
    print('rbffdata shape, width, ,height', RBFdata.shape, x_image_size, y_image_size)
# Get interpolant
    f = RBFInterpolator(RBFdata,np.array(valdata).ravel(), smoothing=100)

# Prepare data arrays to pass to f
    xpoints,ypoints = np.meshgrid(np.arange(0,y_image_size,1),np.arange(0,x_image_size,1))
...
#     newmap = copy(map)
#     norm = matplotlib.colors.Normalize(vmin=0,vmax=224,clip = True) #Normalize data to (-100,0)
