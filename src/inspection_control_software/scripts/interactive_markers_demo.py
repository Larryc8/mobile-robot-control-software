...
class InteractiveMarkerDemo(QObject):
    ...
    def __init__(self):
        ...
        self.menu_handler.insert("Reset Position", parent=sub_menu_handle, callback=self.process_feedback)
        self.menu_handler.insert("Log Status", parent=sub_menu_handle, callback=self.process_feedback)
        ...
        self.move_base_goal_sub = rospy.Subscriber("/move_base_simple/goal", PoseStamped, self.external_goal_callback)
    ...
    def set_user_operation(self, user_operation):
        ...
        if self.__user_operation == userOperation.CREATEMAP or self.__user_operation == userOperation.LOADMAP:
    ...
    def update_markers(self, current_poin_id, next_point_id, point_state):
        for p in self.__interactive_markers.values():
            if p.name == current_poin_id:
                ...
                self.update_color(p.name, 1.0, 0.5, 0.3, 0.8)
    ...
    def update_widgets(self):
        ...
        if self.__isUpdated:
            ...
            for p in self.__interactive_markers.values():
                ...
                quat_list = [p.pose.orientation.x, p.pose.orientation.y, p.pose.orientation.z, p.pose.orientation.w]
    ...
    def process_feedback(self, feedback):
        ...
        if feedback.event_type == InteractiveMarkerFeedback.POSE_UPDATE:
            rospy.loginfo(f"{s}: pose changed to {feedback.pose.position.x:.2f}, {feedback.pose.position.y:.2f}, {feedback.pose.position.z:.2f}")
    ...
    def external_goal_callback(self, msg):
        ...
        rospy.loginfo(f"Received external goal at ({msg.pose.position.x:.2f}, {msg.pose.position.y:.2f}). Updating marker.")
    ...
    def update_color(self, name, r, g, b, a=1):
        ...
        if name in self.__interactive_markers:
            int_marker = self.__interactive_markers[name]
    ...
    def make_6dof_marker(self, name, description, position, img=''):
            ...
            text_marker.color = ColorRGBA(r=0, g=0.0, b=0.0, a=1.0)#ColorRGBA(r=1.0, g=1.0, b=1.0, a=1.0) # Solid white
            ...
            self.__interactive_markers[int_marker.name] = int_marker
